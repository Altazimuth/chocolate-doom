			...
			else if (cht_CheckCheat(&cheat_mus, ev->data2))
			{
				...
				else
				{
					...
					if (((buf[0] - '1') * 9 + buf[1] - '1') > 31)
			          ...
			          && cht_CheckCheat(&cheat_noclip, ev->data2)) || (gamemission != doom && cht_CheckCheat(&cheat_commercial_noclip, ev->data2)))
			{
				// Noclip cheat.
...
void ST_createWidgets(void)
{
	...
	for (i = 0; i < 6; i++)
	{
		STlib_initMultIcon(&w_arms[i],
		                   ST_ARMSX + (i % 3) * ST_ARMSXSPACE, ST_ARMSY + (i / 3) * ST_ARMSYSPACE, arms[i], (int*)&plyr->weaponowned[i + 1], &st_armson);
	}
