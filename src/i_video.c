...
static void LoadDiskImage(void)
{
	...
	V_DrawPatch(SCREENWIDTH - LOADING_DISK_W, SCREENHEIGHT - LOADING_DISK_H, 0, disk);
	...
	{
		memcpy(disk_image + LOADING_DISK_W * y, screens[0] + SCREENWIDTH * (y + yoffset) + xoffset, LOADING_DISK_W);
	}
...
void I_GetEvent(void)
{
	...
	while (SDL_PollEvent(&sdlevent))
	{
		...
		if (!window_focused && (sdlevent.type == SDL_MOUSEMOTION || sdlevent.type == SDL_MOUSEBUTTONDOWN || sdlevent.type == SDL_MOUSEBUTTONUP))
		{
			continue;
		}
...
static boolean BlitArea(int x1, int y1, int x2, int y2)
{
	...
	{
		I_InitScale(screens[0], (byte*) screenbuffer->pixels + (y_offset * screenbuffer->pitch) + x_offset, screenbuffer->pitch);
...
static void UpdateRect(int x1, int y1, int x2, int y2)
{
	...
	if (BlitArea(x1, y1, x2, y2))
	{
		...
		SDL_UpdateRect(screen, x1_scaled, y1_scaled, x2_scaled - x1_scaled, y2_scaled - y1_scaled);
	}
}

void I_BeginRead(void)
{
	byte* screenloc = screens[0] + (SCREENHEIGHT - LOADING_DISK_H) * SCREENWIDTH + (SCREENWIDTH - LOADING_DISK_W);
	int y;
	...
	for (y = 0; y < LOADING_DISK_H; ++y)
	{
		memcpy(saved_background + y * LOADING_DISK_W, screenloc, LOADING_DISK_W);
		memcpy(screenloc, disk_image + y * LOADING_DISK_W, LOADING_DISK_W);
		
		screenloc += SCREENWIDTH;
	}
	
	UpdateRect(SCREENWIDTH - LOADING_DISK_W, SCREENHEIGHT - LOADING_DISK_H, SCREENWIDTH, SCREENHEIGHT);
}
...
void I_EndRead(void)
{
	...
	for (y = 0; y < LOADING_DISK_H; ++y)
	{
		memcpy(screenloc, saved_background + y * LOADING_DISK_W, LOADING_DISK_W);
...
void I_SetWindowCaption(void)
{
	...
	buf = Z_Malloc(strlen(gamedescription) + strlen(PACKAGE_STRING) + 10, PU_STATIC, NULL);
...
void I_SetWindowIcon(void)
{
	...
	{
		if (icon_data[i * 3] != 0x00 || icon_data[i * 3 + 1] != 0x00 || icon_data[i * 3 + 2] != 0x00)
		{
			mask[i / 8] |= 1 << (7 - i % 8);
		}
	}
...
static boolean AutoAdjustFullscreen(void)
{
	...
	for (i = 0; modes[i] != NULL; ++i)
	{
		...
		diff = (screen_width - modes[i]->w) * (screen_width - modes[i]->w) + (screen_height - modes[i]->h) * (screen_height - modes[i]->h);
...
static void I_AutoAdjustSettings(void)
{
	...
	if (screen_width != old_screen_w || screen_height != old_screen_h || screen_bpp != old_screen_bpp)
	{
...
static void SetScaleFactor(int factor)
{
	if (fullscreen)
	{
		...
		for (i = 0; modes[i] != NULL; ++i)
		{
			...
			if ((scrmode->width / SCREENWIDTH) != factor || (scrmode->height / SCREENHEIGHT) != factor)
			{
				continue;
			}
		...
		if (best_mode == NULL)
		{
			I_Error("No fullscreen graphics mode available to support " "%ix scale factor!", factor);
		}
...
static void SetVideoMode(screen_mode_t* mode, int w, int h)
{
	...
	{
		screenbuffer = SDL_CreateRGBSurface(SDL_SWSURFACE, mode->width, mode->height, 8, 0, 0, 0, 0);
...
void I_InitGraphics(void)
{
	...
	else
	{
		...
		if (screen_mode == NULL)
		{
			I_Error("I_InitGraphics: Unable to find a screen mode small " "enough for %ix%i", w, h);
		}
		
		if (w != screen_mode->width || h != screen_mode->height)
		{
			printf("I_InitGraphics: %s (%ix%i within %ix%i)\n", WindowBoxType(screen_mode, w, h), screen_mode->width, screen_mode->height, w, h);
		}
	...
	native_surface = screen == screenbuffer && !SDL_MUSTLOCK(screen) && screen_mode == &mode_scale_1x && screen->pitch == SCREENWIDTH && aspect_ratio_correct;
