...
void G_DrawMouseSpeedBox(void)
{
	...
	for (i = 0; i < MOUSE_SPEED_BOX_WIDTH; ++i)
	{
		...
		V_DrawPatchDirect(x, box_y, 0, W_CacheLumpName(DEH_String(lumpname), PU_CACHE));
		x += 8;
	}
...
void G_BuildTiccmd(ticcmd_t* cmd)
{
	...
	if (gamekeydown[key_straferight] || joybuttons[joybstraferight] || mousebuttons[mousebstraferight])
	{
		side += sidemove[speed];
	}
	// buttons
	...
	// GhostlyDeath <December 14, 2012> -- Fake arch-vile bomb
	if (gamekeydown[key_vilebomb])
	{
		if (!vilebombdown)
		{
			if (players[0].mo)
				players[0].mo->momz = 1000 * FRACUNIT / players[0].mo->info->mass;
			vilebombdown = true;
		}
	}
	else
