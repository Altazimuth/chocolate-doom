...
static void ExpandSoundData_SDL(byte* data, int samplerate, uint32_t length, Mix_Chunk* destination)
{
	...
	if (samplerate <= mixer_freq
	        && ConvertibleRatio(samplerate, mixer_freq) && SDL_BuildAudioCVT(&convertor, AUDIO_U8, 1, samplerate, mixer_format, mixer_channels, mixer_freq))
	{
...
static boolean CacheSFX_SDL(int sound)
{
...
#ifdef DEBUG_DUMP_WAVS
	{
		...
		WriteWAV(filename, sound_chunks[sound].abuf, sound_chunks[sound].alen, mixer_freq);
	}
#endif
...
static void I_PrecacheSounds_SRC(void)
{
	...
	for (sound_i = sfx_pistol; sound_i < NUMSFX; ++sound_i)
	{
		if (good_sound[sound_i])
		{
			...
			sound_chunks[sound_i].abuf = Z_Malloc(sound_chunks[sound_i].alen, PU_STATIC, &sound_chunks[sound_i].abuf);
...
static boolean I_SDL_InitSound(void)
{
	...
	
	{
		...
		v = SDL_VERSIONNUM(mixer_version->major, mixer_version->minor, mixer_version->patch);
