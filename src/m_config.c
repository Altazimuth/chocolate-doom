...
static default_t extra_defaults_list[] =
{
	...
	//!
	// Self Inflicted Arch-Vile Wound
	//
	
	CONFIG_VARIABLE_KEY(key_vilebomb, key_vilebomb),
};
...
void M_LoadDefaults(void)
{
	...
	{
		extra_defaults.filename = malloc(strlen(configdir) + strlen(PACKAGE_TARNAME) + 10);
		sprintf(extra_defaults.filename, "%s%s.cfg", configdir, PACKAGE_TARNAME);
	}
	
	LoadDefaultCollection(&doom_defaults);
	LoadDefaultCollection(&extra_defaults);
	
	// GhostlyDeath <December 14, 2012> -- CRL Config
	extra_defaults.filename = malloc(strlen(configdir) + strlen("chocorenderlimits") + 10);
	sprintf(extra_defaults.filename, "%s%s.cfg", configdir, "chocorenderlimits");
...
void M_SetConfigDir(void)
{
	...
	if (homedir != NULL)
	{
		...
		sprintf(configdir, "%s%c.%s%c", homedir, DIR_SEPARATOR, PACKAGE_TARNAME, DIR_SEPARATOR);
...
static int SystemHasKeyboard(void)
{
	...
	if (RegOpenKeyExW(HKEY_CURRENT_USER, L"\\Software\\Microsoft\\Shell", 0, KEY_READ, &key) != ERROR_SUCCESS)
	{
		return 0;
	}
	...
	if (RegQueryValueExW(key, L"HasKeyboard", NULL, &valtype, (LPBYTE) & result, &valsize) != ERROR_SUCCESS)
	{
		result = 0;
	}
	// Close the key
...
void M_ApplyPlatformDefaults(void)
{
...
#ifdef __MACOSX__
	{
		...
		if (SDL_VERSIONNUM(v->major, v->minor, v->patch) < SDL_VERSIONNUM(1, 2, 11))
		{
