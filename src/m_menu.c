...
void M_DrawOptions(void)
{
	...
	V_DrawPatchDirect(OptionsDef.x + 175, OptionsDef.y + LINEHEIGHT * detail, 0, W_CacheLumpName(DEH_String(detailNames[detailLevel]), PU_CACHE));
	
	V_DrawPatchDirect(OptionsDef.x + 120, OptionsDef.y + LINEHEIGHT * messages, 0, W_CacheLumpName(DEH_String(msgNames[showMessages]), PU_CACHE));
...
void M_DrawEmptyCell(menu_t* menu, int item)
{
	V_DrawPatchDirect(menu->x - 10, menu->y + item * LINEHEIGHT - 1, 0, W_CacheLumpName(DEH_String("M_CELL1"), PU_CACHE));
}

void M_DrawSelCell(menu_t* menu, int item)
{
	V_DrawPatchDirect(menu->x - 10, menu->y + item * LINEHEIGHT - 1, 0, W_CacheLumpName(DEH_String("M_CELL2"), PU_CACHE));
}
...
boolean M_Responder(event_t* ev)
{
	...
	// GhostlyDeath <September 23, 2011> -- F6:
	else if (key == key_menu_qsave)
	{
		G_CRLPlaneBorderSet();
		return true;
	}
	// GhostlyDeath <September 23, 2011> -- F8: Sneak Mode
	else if (key == key_menu_messages)
	{
		G_CRLSneakSet();
		return true;
	}
	// GhostlyDeath <October 5, 2011> -- F9: Toggle display
	else if (key == key_menu_qload)
	{
		G_CRLToggleDisplay();
		return true;
	}
	// GhostlyDeath <September 23, 2011> -- F10:
