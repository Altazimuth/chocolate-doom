...
static void LoadOperatorData(int operator, genmidi_op_t* data, boolean max_level)
{
	int level;
...
static void SetVoiceVolume(opl_voice_t* voice, unsigned int volume)
{
	...
	full_volume = (volume_mapping_table[voice->note_volume]
	               * volume_mapping_table[voice->channel->volume] * volume_mapping_table[current_music_volume]) / (127 * 127);
...
static int CompareChannelPriorities(opl_channel_data_t* chan1, opl_channel_data_t* chan2)
{
	// TODO ...
...
static opl_voice_t* ReplaceExistingVoice(opl_channel_data_t* channel)
{
	...
	{
		if (rover->current_instr_voice != 0 || (rover->channel > channel && CompareChannelPriorities(channel, rover->channel) > 0))
		{
			result = rover;
			break;
		}
...
static void VoiceKeyOn(opl_channel_data_t* channel, genmidi_instr_t* instrument, unsigned int instrument_voice, unsigned int key, unsigned int volume)
{
	opl_voice_t* voice;
