...
int G_ClearMaxVis()
{
	...
	players[consoleplayer].message = DEH_String("Cleared max!");
	return 0;
}
...
int G_MoveMaxVis()
{
	...
	players[consoleplayer].message = DEH_String("Move to max");
	return 0;
}
...
int G_CRLWriteLog()
{
	...
	if (log)
	{
		...
		fprintf(log, "Most vis %i at x=%i y=%i vz=%i a=%i am=%i\n",
		        MaxPlaneCount, (int)(MaxPlanePos[0]) >> FRACBITS, (int)(MaxPlanePos[1]) >> FRACBITS, (int)(MaxPlanePos[1]) >> FRACBITS, nwtest, nwfactor);
		...
		if (players[displayplayer].mo)
		{
			...
			fprintf(log, "Play x=%i y=%i vz=%i a=%i am=%i\n",
			        ...
			        (int)(players[displayplayer].mo->y) >> FRACBITS, (int)(players[displayplayer].viewz) >> FRACBITS, nwtest, nwfactor);
		}
		// openings:
...
int g_crlvisbordermode = 0;

/* G_CRLPlaneBorderSet() -- Sets plane bordering */
int G_CRLPlaneBorderSet()
{
	g_crlvisbordermode++;
	if (g_crlvisbordermode >= 4)
		g_crlvisbordermode = 0;
		
	switch (g_crlvisbordermode)
	{
		case 1:
			players[consoleplayer].message = DEH_String("Mode 1: Normal");
			break;
		case 2:
			players[consoleplayer].message = DEH_String("Mode 2: OverNormal");
			break;
		case 3:
			players[consoleplayer].message = DEH_String("Mode 3: Borders");
			break;
		default:
			players[consoleplayer].message = DEH_String("Mode 0: No Markers");
			break;
	}
}

int g_crldisplaymode = 0;

/* G_CRLToggleDisplay() -- Toggles display mode */
int G_CRLToggleDisplay()
{
	g_crldisplaymode++;
	if (g_crldisplaymode >= 3)
		g_crldisplaymode = 0;
		
	switch (g_crldisplaymode)
	{
		case 0:
			players[consoleplayer].message = DEH_String("Standard HUD");
			break;
		case 1:
			players[consoleplayer].message = DEH_String("Overflows Only");
			break;
		case 2:
		default:
			players[consoleplayer].message = DEH_String("None");
			break;
	}
}

int g_crlsneakmode = 0;

/* CRLSneakSet() -- Toggles sneaking mode */
int G_CRLSneakSet()
{
	g_crlsneakmode = !g_crlsneakmode;
	
	if (g_crlsneakmode)
		players[consoleplayer].message = DEH_String("Sneaking!");
	else
		players[consoleplayer].message = DEH_String("Not sneaking!");
}

int g_NoFlashyPurpleHOM = 0;

/* G_CRLToggleHOMFlash() -- Toggles HOM flashing */
int G_CRLToggleHOMFlash()
{
	g_NoFlashyPurpleHOM = !g_NoFlashyPurpleHOM;
	if (g_NoFlashyPurpleHOM)
		players[consoleplayer].message = DEH_String("Black HOM!");
	else
		players[consoleplayer].message = DEH_String("Purple HOM!");
}
...
static void DrawRenderLimits(void)
{
	...
	char* status;
	int rl_pos;
	int flash_color, flash_color2, flashcolor;
	
	// Skip drawing them?
	if (g_crldisplaymode == 2)
		return;
	...
	if (!g_crldisplaymode || (numplats > MAXPLATS))
	{
	...
	if ((int)(lastvisplane - visplanes) < NORMMAXVISPLANES)
	{
		flashcolor = 0;
		status = "";
	}
	else if ((int)(lastvisplane - visplanes) == NORMMAXVISPLANES)
	{
		flashcolor = flash_color2;
		// visplane
		status = "===";
	}
	else
	...
	if (!g_crldisplaymode || ((lastvisplane - visplanes) > NORMMAXVISPLANES))
	{
		if (flashcolor)
			V_DrawRect(0, rl_pos - 10, 200, 8, flashcolor);
			
		sprintf(buffer, "PLN: %i/%i (MAX: %i) %s ", lastvisplane - visplanes, NORMMAXVISPLANES, MaxPlaneCount, status);
	...
	if (!g_crldisplaymode || (ActualDrawSegs >= NORMMAXDRAWSEGS))
	{
		if (flashcolor)
			V_DrawRect(0, rl_pos - 20, 200, 8, flashcolor);
			
		sprintf(buffer, "SEG: %i/%i %s", ActualDrawSegs, NORMMAXDRAWSEGS, status);
		M_WriteText(0, rl_pos - 20, buffer);
	}
	// opening
	if (!g_crldisplaymode || (lastopening - openings) > MAXOPENINGS)
	{
		sprintf(buffer, "OPN: %i/%i", lastopening - openings, MAXOPENINGS);
		M_WriteText(0, rl_pos - 30, buffer);
	}
	// vissprites
	if (!g_crldisplaymode || ((vissprite_p - vissprites) + 1) > MAXVISSPRITES)
	{
