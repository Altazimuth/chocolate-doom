...
extern line_t* dc_linedef;
extern side_t* dc_sidedef;
...
void R_RenderMaskedSegRange(drawseg_t* ds, int x1, int x2)
{
	...
	{
		if (dc_linedef)
			if (!dc_linedef->Sneaky)
				SkipActualDraw = 1;
				
		if (dc_sidedef)
			if (!dc_sidedef->Sneaky)
				SkipActualDraw = 1;
	}
...
line_t* dc_linedef = NULL;
side_t* dc_sidedef = NULL;
...
void R_StoreWallRange(int start, int stop)
{
	...
	if (frontsector)
	{
		xx |= frontsector->Sneaky;
		yy |= frontsector->Flashy;
	}
	if (backsector)
	{
		xx |= backsector->Sneaky;
		yy |= backsector->Flashy;
	}
	// render it
	if (markceiling)
		ceilingplane = R_CheckPlane(ceilingplane, rw_x, rw_stopx - 1, xx, yy);
		
	if (markfloor)
		floorplane = R_CheckPlane(floorplane, rw_x, rw_stopx - 1, xx, yy);
