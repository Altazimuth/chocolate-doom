...
int TXT_Init(void)
{
	...
	screen = SDL_SetVideoMode(TXT_SCREEN_W * font->w, TXT_SCREEN_H * font->h, 8, 0);
...
static inline void UpdateCharacter(int x, int y)
{
	...
	s = ((unsigned char*)screen->pixels) + (y * font->h * screen->pitch) + (x * font->w);
...
void TXT_UpdateScreenArea(int x, int y, int w, int h)
{
	...
	SDL_UpdateRect(screen, x * font->w, y * font->h, (x_end - x) * font->w, (y_end - y) * font->h);
}
